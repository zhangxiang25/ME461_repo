...
// ----- code for CAN end here -----
//ZHX lab6 ex1 difine variables
float LeftWheel=0.0;
float RightWheel=0.0;
float distanceL=0.0;
float distanceR=0.0;

//ZHX LAB6 EX2 define variables
float uLeft=5.0;
float uRight=5.0;
float PosLeft_K=0.0;
...
//ZHX LAB6 EX3&4 define variables
//float Kp=3;
//float Ki=25;
float Kturn=3;
float eturn=0.0;
...
//ZHX lab7 ex1/lab6 ex8 paste from LAB4
...
// ZHX LAB7 ex2 Needed global Variables
...
//JLS: lab 7 exercise 4 -- difference between the left wheel's angle in radians & the right wheel's angle in radians
float WhlDiff = 0;
...
void main(void)
{
    ...
    //ZHX LAB7 ex1 paste from LAB4 using EPWM5 to trigger ADCA channel
    EALLOW;
    ...
    // ZHX from lab3 following lines are copied from Lab 3 for intialize the pinmux for EPWM2. EPWM2A and 2B drive the robot's DC motors.EPWM2A controls right motor and EPWM2B controls left motor
...
__interrupt void ADCA_ISR (void) {
    ...
    //ZHX lab7 ex1.5 Spi transmission and reception of the three accelerometer reading and the 3 gyro readings
...
__interrupt void SWI_isr(void) {
    ...
    //ZHX LAB7 ex3 the transfer function is 125s/(s+125), and the discrete transfer function is（100z-100)/(z-0.6)=Y(z)/V(z)
    ...
    //ZHX LAB7 ex3 the state feedback controller equation is u=-kx states are tile,gyro,average of left and right motor velocities and gyrorate_dot
    ubal= -K_1*tilt_value-K_2*gyro_value - K_3*(vel_Left+vel_Right)/2.0-K_4*gyrorate_dot;
    //ZHX LAB7 ex3 ubal is controleffort for both motors
    //    uLeft=ubal/2;
    //    uRight=ubal/2;
    ...
    // ZHX lab7 ex4 saturate turn between -4 and 4
    if (turn>4){
        turn=4;
    }
    if (turn<-4){
    ...
    //ZHX lab7 ex5 saturate IK_eSpeed at its current value when absolutue value of ForwardBackwardCommand is larger than 3
    if(fabs(ForwardBackwardCommand) > 3){
        IK_eSpeed=IK_eSpeed_1;
    }
    // ZHx lab7 ex5 saturate ForwardBackwardCommand between -4 and 4.
    ...
    //ZHX lab7 ex5 copy fron interrupt function from lab 6(kinematic equations + communication with labview)
    theta_l=LeftWheel;
    theta_r=RightWheel;
    ...
    x_dot_prev=x_dot;
    y_dot_prev=y_dot;
    // ZHX LAB6 ex5  press 'q', turn+0.05. press 'r', turn-0.05. press '3', Vref+0.1. press's', Vref-0.1.
    //ZHX COPY FROM LAB6 EX5
    if (NewLVData == 1) {
        NewLVData = 0;
        float dummy = 0.0;
...
__interrupt void cpu_timer2_isr(void)
{
    ...
    //    //ZHX lab6 ex7 right wall following
    ...
    //
    //    //ek_L=Vref-VLeftK; ZHX lab6 ex3 from decoupled PI controller ek=Vref-vk
    //
    //    ek_L=Vref-VLeftK-Kturn*eturn; //ZHX lab6 ex4 implement a steering controller. turn setpoint controls the amount by which motor's speed exceed the other motor's speed.
    //
    ...
    //
    //
    //    // ZHX lab6 ex3 To prevent integral wind-up, we implement an anti-windup controller,in other words stop integrating when the control effort is saturated with the limit of 10 and -10。
    //    if (uLeft>=10){
    //        uLeft=10;
...
__interrupt void SPIB_isr(void) {
    //ZHX lab7 ex1.6 pull the MPU-9250's slave select high
    GpioDataRegs.GPCSET.bit.GPIO66 = 1;
    ...
    //LAB7 EX2 Code to be copied into SPIB_ISR interrupt function after the IMU measurements have been collected.
    ...
    if(calibration_state == 0){
        ...
        /*LAB7 EX2--------------Kalman Filtering code start---------------------------------------------------------------------*/
...
// ----- code for CAN end here -----
//ZHX lab6 ex1 paste code from manual
void init_eQEPs(void) {
...
// ZHX lab6 EX1 Optical encoder is in the end of robot's motor.There are 100 silts on the wheel inside the enclosure and encoder sense each slit.So one rotation of the motor creates 100 square wave period per rotation
float readEncLeft(void) {
    int32_t raw = 0;
